...
import { CreateReturnFunc } from '../../BackendApiCalls/ReturnApi'
import CustomAlert from '../../Partials/CustomAlert'
import InputField from '../../UI Elemets/InputField'

export default function AddReturnModal({ Modal, setModal }) {

    const ModalRef = useRef()
    const { dummyState, setdummyState, SelectedStore, ReturnType } = useContext(GlobalStateContext)
    ...
    const handleSubmit = (e) => {
        ...
        if (SelectedStore) {
            ...
            CreateReturnFunc(data).then(() => {
                CustomAlert('Return created successfully!', 'success')
            }).catch((error) => {
                if (error.response.status === 404) {
                    CustomAlert('Order not found!', 'error')

                }
                else if (error.response.status === 406) {
                    CustomAlert('Invalid return quantity!', 'error')
    ...
    return (
        Modal && <div>
            ...
            <div id="popup-modal" tabIndex="-1" className="fixed  top-0 left-0 right-0 z-50 p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full" >
                <div className=" relative  lg:w-[32rem] h-full md:h-auto mx-auto mt-16" ref={ModalRef}>
                    <div className="relative bg-white rounded-lg shadow">
                        ...
                        <div className="max-h-[37rem] overflow-auto scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300 scrollbar-medium rounded-scrollbar flex flex-col px-5 pb-5">
                            <div className='border-b py-1'>
                                <h1 className='text-dark-primary text-lg font-semibold relative bottom-3'>Add Return</h1>
                            </div>
