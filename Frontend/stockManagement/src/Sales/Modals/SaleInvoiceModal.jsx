...
import { SingleSaleFunc } from '../../BackendApiCalls/SaleApi'
import GlobalStateContext from '../../Context/GlobalStateContext'
import Loader from "../../Partials/Loader"
import html2pdf from 'html2pdf.js';

export default function SaleInvoiceModal({ Modal, setModal, ActionId, setActionId }) {
    ...
    const InvoiceLabels = ["Order Id", "Ordered at"]

    useEffect(() => {
        SingleSaleFunc(ActionId).then((data) => {
            setInvoiceLabelData([data.orderId, formatDate(data.created)])
            setInvoiceData(data)
            setTimeout(() => {
                setloading(false)
            }, 1000);
    ...
    return (
        Modal && <div>
            ...
            <div id="popup-modal" tabIndex="-1" className="fixed  top-0 left-0 right-0 z-50 p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full" >
                <div className=" relative  lg:w-[40rem] h-full md:h-auto mx-auto mt-4" >
                        ...
                        {loading ? <Loader /> : <div className=" flex flex-col px-5  ">
                            ...
                            <div className='max-h-[33rem]  overflow-auto scrollbar-thin scrollbar-track-gray-100 scrollbar-thumb-gray-300 scrollbar-medium rounded-scrollbar ' id="invoice">
                                <Invoice
                                    ...
                                    InvoiceItemLabels={["Item", "Quantity", "Item Price", "Notes"]}
                                    InvoiceItemData={[InvoiceData.inventoryData, InvoiceData.quantity, InvoiceData.pricePerItem, InvoiceData.notes]}
                                    InvoiceTotalAmountLabels={[InvoiceData.extraExpenseData, "Net extra charges on Order", "Net extra charges per Item", "Total  Amount"]}
                                    InvoiceTotalAmountData={[InvoiceData.extraExpenseData, InvoiceData.totalExpenseAmount, (parseFloat(InvoiceData.totalExpenseAmount) / parseFloat(InvoiceData.quantity)).toFixed(2), InvoiceData.totalOrderAmount]}
