...
import { AddCommonSaleExpenseFunc } from '../../BackendApiCalls/SaleApi'

export default function AddCommonSaleExpenseModal({ Modal, setModal }) {

    const ModalRef = useRef()
    const { dummyState, setdummyState, SelectedStore, SaleType } = useContext(GlobalStateContext)
    ...
    const handleSubmit = (e) => {
        ...
        if (SelectedStore) {
            ...
            if (data['fromDate'] > data['toDate']) {
                ...
                AddCommonSaleExpenseFunc(data)
                    .then(() => {
                        CustomAlert('Expenses merged  successfully!', 'success')
                    })
                    .catch((error) => {
                        if (error.response.status === 404) {
                            CustomAlert('No Sales found!', 'error')
                        }
                        else {
